...
The external identity provider is Keycloak. "Creating an external identity user" below means an HTTP call that creates an account in a Keycloak realm, and the matching compensation deletes it. See [Keycloak Identity Provider](Keycloak-Identity-Provider.md).
